...
def week_birthday(conn, user):
    user = str(user)
    sql = "SELECT * FROM contacts WHERE (DAYOFYEAR(birthday)-DAYOFYEAR(CURRENT_DATE())<=7 AND DAYOFYEAR(birthday)>=DAYOFYEAR(CURRENT_DATE())) OR (DAYOFYEAR(CURRENT_DATE())+365-DAYOFYEAR(birthday)<=7 AND DAYOFYEAR(CURRENT_DATE())>DAYOFYEAR(birthday))"
    with conn.cursor() as cursor:
        cursor.execute(sql)
        result = cursor.fetchall()
        return result